...
需求：击败陨石 https://class.coursera.org/interactivepython-005/human_grading/view/courses/972530/assessments/35/results/mine
...
def process_sprite_group(sprite_set,canvas):
    ...
    if(len(remove_list)>0):
        for sp in remove_list:
            sprite_set.remove(sp);            
    for sp in  sprite_set:
        sp.draw(canvas);
...
def group_group_collide(group1,group2):
    ...
    hit_num = 0
    for sp1 in group1:
        if(group_collide(sp1,group2)):
            hit_num +=1;
            remove_list.append(sp1);
...
class Sprite:
    ...
    def collide(self,other_object):
        ...
        if(dist(self.pos, other_object.pos)<self.get_radius()+other_object.get_radius()):
